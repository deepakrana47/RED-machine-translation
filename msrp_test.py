...
# def generate_fixed_vector_sentv(nn, x, nfeat, pool_size):
#     o = []
#     for i in range(0, len(x), 2):
#         temp1 = sum_vector(x[i])
#         temp2 = sum_vector(x[i+1])
#         if nfeat == 1:
#             feat = get_n_feature(' '.join([x[i]['words'][j] for j in x[i]['words']]), ' '.join([x[i+1]['words'][j] for j in x[i+1]['words']]))
#             o.append(np.concatenate((temp1[len(temp1) - 1].reshape(len(temp1[len(temp1) - 1])), temp2[len(temp2) - 1].reshape(len(temp2[len(temp2) - 1])), feat)))
#         else:
#             o.append(np.concatenate((temp1[len(temp1) - 1], temp2[len(temp2) - 1])).reshape(len(temp1[len(temp1) - 1])+len(temp2[len(temp2) - 1])))
#     return o
...
def init_model(var_file):
    ...
    if len(a) == 5:
        isize, hsize, w, b, g = a
        nn = stack_RAE(input_size=isize, hidden_size=hsize, hidden_activation=tanh, hidden_deactivation=dtanh, output_activation=tanh, output_deactivation=dtanh)
    else:
...
if __name__ == "__main__":
    var_file = ['/media/zero/41FF48D81730BD9B/DT_RAE/IMPLEMENTATION/weights/RAE_50_50_1_rmsprop_dep_h_tanh_tanh/model_variables.pickle'
               ]
    pool_size = 10
    num_feature = [0,1]
    stp = [1]
