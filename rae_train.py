...
def create_model(wfname, wload, model_type, op_method, i_size, o_size, neta, wpresent, logg, vector):
    ...
    if os.path.isfile(wfname) or os.path.isfile(wload):
        ans = raw_input(wload+"\nInput model file exist wanted to load(y/n) :") if wload else raw_input(wload+"\nSame model file exist wanted to load(y/n) :")
        if ans == 'y' or ans == 'Y':
            ...
            print "model loaded ..."
            return a
    a = RAE(input_size=i_size, hidden_size=o_size, optimization=opt,hidden_activation=args['hactiv'][0],hidden_deactivation=args['hactiv'][1],output_activation=args['oactiv'][0],output_deactivation=args['oactiv'][1], wpresent=wpresent, log_file=logg, log=1, vector=vector)
    return a
...
def main_train(args):
    ...
    words_data_check = pickle.load(open("/media/zero/41FF48D81730BD9B/DT_RAE/data/pickle/110.pickle", 'rb'))
...
if __name__ == '__main__':
    ...
    args = {
            ...
            'src': '',
            'wload':'',
            'wfname':'/media/zero/41FF48D81730BD9B/DT_RAE/IMPLEMENTATION/weights/spars_stack_' ,
            }
