...
class preprocess:
    ...
    def __init__(self, parsing_type, structure_type, stopword, wvect=None):
        ...
        elif parsing_type == 'ladj':
            self.processing = self.ladj_processing
        elif parsing_type == 'lradj':
            self.processing = self.lradj_processing
        else:
            print "Provide proper parsing type"
            exit()
        ...
        self.wd_vect = wvect
        self.stp = stopword
        self.stopwds = [i.strip('\n') for i in open('/media/zero/41FF48D81730BD9B/DT_RAE/Chunk_rnn/config/stopword.txt', 'r')]

    def addvector(self, words):
        return {i:self.wd_vect.get_word_vect(words[i].lower()) for i in words}
    ...
    def remove_stopwords(self, word_data):
        wd = {}
        oth = {}
        for i in word_data:
            if type(i) == int:
                ...
                oth[i] = word_data[i]
        word_data = self.removing_stopword(wd)
        word_data.update(oth)

    def get_wpresent(self, wp):
        t = []
        for i in wp:
            t += [j for j in i]
        return t

    def get_word_only(self, word_data):
        return {i: word_data[i]for i in word_data if type(i) == int and word_data[i]['pid'] != -1}
    ...
    def process_words_data(self, wds):
        ...
        for iword in range(len(wds)):
            if wds[iword] == -2 and not wds[iword]: continue
            word_data = self.remove_stopwords(wds) if self.stp == 0 else wds[iword]

            h_index, h_vect, wp, words = self.processing(word_data)

            wpresent = list(set(wpresent + self.get_wpresent(wp)))

            vects = self.addvector(words)

            wd_extra.append({'w_size': len(words), 'h_vect': h_vect, 'words':words, 'vects': vects, 'wp': wp})

            if counts%10000 == 0: print "%d/%d are processed ...." % (len(wd_extra), len(wds))
            counts += 1
    ...
    def chk_processing(self, word_data):
        wd = {}
        for i in word_data:
            if type(i) == int and word_data[i]['pid'] != -1:
                wd[i] = word_data[i]
        p = []; dep = []
        for i in wd:
            p.append(wd[i]['pid'])
        ...
        words = {}
        for i in sorted(wd):
            words[count] = wd[i]['word']
            count += 1
        return None, h_vect, wp, words
    ...
    def syn_processing(self, wrd):
        ...
        words={}
        for i in sorted(wd):
            words[count] = wd[i]['word']; count+=1
        return h_index, h_vect, wp, words

    def dep_processing(self, word_data):
        wd = self.get_word_only(word_data)
        wids = sorted(wd)
        p=[];d=[]
        for i in wids:
            p.append(wd[i]['pid'])
        ...
        h_index, h_vect, wp , hh_index = dep_2_hid_var(p, dep_order, d1, wids)
        words = {i: wd[wids[i]]['word'] for i in range(len(wids))}
        return h_index, h_vect, wp, words

    def ladj_processing(self, word_data):
        h_index=[]
        h_vect=[]
        wp=[]
        wd = {}
    ...
    def lradj_processing(self, word_data):
        h_index=[]
        h_vect=[]
        wp=[]
        wd = {}
        ...
        count = 1
        wlen = len(wd)
        h_vect.append([count-1, count]); wp.append([0,1])
        while count < wlen-1:
            h_vect.append([count-1,count,count+1])
            wp.append([-1,0,1])
            count+=1
        h_vect.append([count-1, count]); wp.append([-1,0])
        count = 0
        words = {}
        for i in sorted(wd):
            words[count] = wd[i]['word']
